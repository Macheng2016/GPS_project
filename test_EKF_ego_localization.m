%this is a test program for EKF mitigation of the multipath
%error on the state estimation. The methodology is to EKF to
% represent the state vector as well as the multipath of each satellite.
% Cooperative map matching should be
% added later as additional measurement to improve localization accuracy.
% For example, the CMM can use the ego localization results as input and
% give the estimation position of each vehicle, then merge with the ego
% results, however, information dependence should be handled with some
% method to avoid over confidence.
...
Ns=400;  % #of simulation time points
Np=500; % #of particles
...
for i = 1:Ns  %draw the first step
    ...
    [svxyzmat{k},svid{k},elevation_angle{k}] = gensv(usrxyz{k},t,0,orgxyz,6);   % here 0 is the mask angle to determine which satellites are visible to the vehicles, macshen
   % id=[id;svid(1)]; The last argument is to specify the wanted number of satellite and give the first group of  available satellites %added by macshen
    [prvec{k},adrvec{k},pr_err_vec{k}] = genrng(k,usrxyz{k},svxyzmat{k},svid{k},t,noise_scaling_factor,[],mpmat{k});  %pr_err_vec added by macshen, receiver identification k seems to be redundant here. the magnitude of noise flag is thermal,Tropospheric,SA,Multipath,Ionospheric
     pr_err=[pr_err;pr_err_vec{k}]; %added by macshen
...
if i==1   %for the first time step, initialize particles for ego-state estimation and multipath mitigation
    initialize_EKF_ego([usrenu{1}(1,1),usrenu{1}(1,2),0],[0,0,0],0);
    y(i,:)=EKF_update_ego(prvec{k},svxyzmat{k},sigma_thermal2,orgxyz);   %pf update given measurement
else  %second step and so on

    EKF_prediction_ego(sigma_a2,sigma_b2,sigma_d2,sigma_m2,sigma_md2,deltat);  %in this step, the multipath is not changed, it should be considered whether this will cause over confidence.
    y(i,:)=EKF_update_ego(prvec{k},svxyzmat{k},sigma_thermal2,orgxyz);
end
state=[state,EKF.mu];
...
%     deter_error(i)=cal_deter_error(err_temp);  %calculate the determinant of the covariance of error
    ...
    plotcov2d(EKF.mu(1),EKF.mu(3),[EKF.cov(1,1),EKF.cov(1,3);EKF.cov(3,1),EKF.cov(3,3)],'b',0,0,0,3);  %plot the true position of vehicle as a circle
    %when not Kalman
